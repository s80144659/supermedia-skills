...
- `JsonResource` 的欄位型別與可空性寫在各欄位前 docblock 的 `@var`；寫在 `toArray()` 的 method 層 `@return` 不會進入 schema，`make:resource` 產生的那份直接刪掉。
...
- export 出現 `type: string` 的欄位要逐一核對實際回傳值；Scramble 推導失敗時輸出的 `UnknownType` 繼承 `StringType`，在文件上與真正的字串無法分辨。
